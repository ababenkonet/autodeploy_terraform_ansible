...
CURRENT_IMAGE_HASH="$(sudo docker inspect --format='{{.Id}}' alexbabenkodunice/goods:latest)"

# Проверка наличия новых образов
sudo docker pull alexbabenkodunice/goods:latest

# Получение хеша последней версии образа
LATEST_IMAGE_HASH="$(sudo docker inspect --format='{{.Id}}' alexbabenkodunice/goods:latest)"
